#!/usr/bin/env python3
"""
enrich.py — async domain-level enricher (GitHub-ready)

Quick summary
-------------
- Reads a CSV (default: filtered_companies.csv) with CompanyName/company and/or email columns.
- Performs MX-only mailserver checks (no A-record fallback).
- Scrapes a small set of contact pages once per domain (cached).
- Reuses a single aiohttp session and bounds concurrency with a semaphore.
- Scores addresses so `scraped` and `given` are high-confidence; penalizes disposable & role addresses.
- Outputs enriched.csv and a sample_top100.csv by default.
...
# scoring weights (tuned to prefer scraped/given addresses)
WEIGHTS = {
    "scraped": 50,
    "given": 40,
    "generated": 5,
...
# ---------------------- MX cache (no A fallback) ----------------------


class MXCache:
    def __init__(self, mx_timeout: float = 5.0, threads: int = 8):
    ...
    def _resolve_sync(self, domain: str) -> List[Tuple[int, str]]:
        try:
            ...
            # Do NOT fallback to A records as MX — generates false positives.
            return []
        ...
        for scheme in schemes:
            ...
            for p in CONTACT_PAGES:
                ...
                try:
                    ...
                    # skip errors quietly — counters in caller track failures
                    continue
    return found
...
async def enrich(
    ...
    async def process_row(r: Dict[str, str], session: aiohttp.ClientSession):
        ...
        # use input email if valid
        if in_email:
            ok, norm = validate_syntax(in_email)
            if ok and norm:
                candidate_emails.append((norm.lower(), "given"))

        # only generate minimal guesses when no given email
        ...
        # domains to check (preserve deterministic order)
        domains_to_check: List[str] = []
        for em, _src in candidate_emails:
            if "@" in em:
                domains_to_check.append(em.rsplit("@", 1)[-1].lower())
        if company_present:
        ...
        domains_to_scrape = [d for d in domain_mx_map.keys() if d not in domain_scrape_cache]
    ...
    try:
        # create shared session context — this ensures clean close on errors
...
def parse_args():
    p = argparse.ArgumentParser(description="Async email/domain enricher (no SMTP).")
    p.add_argument("--input", required=False, default="filtered_companies.csv", help="Input CSV (must contain at least CompanyName or email columns)")
    ...
    p.add_argument("--target", type=int, default=None, help="Target number of enriched rows to produce (trim final output)")
