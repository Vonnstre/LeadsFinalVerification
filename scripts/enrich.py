#!/usr/bin/env python3 """ scripts/enrich.py — production-grade, proof-backed email enricher
...
Usage (example): python3 scripts/enrich.py --input filtered_companies.csv --output enriched_verified.csv 
--concurrency 30 --smtp-timeout 8 --mx-timeout 6 --mail-from-list mailfroms.txt --ports 25,587,465

Notes:

This file is designed to be run from a host that allows outbound SMTP (port 25) or from a relay.

If port 25 is blocked, use --skip-smtp to rely on scraping + MX heuristics (less accurate).


Transcripts produced by SMTP probes are written under ./transcripts/ by default. Each enriched CSV row includes _smtp_transcript_path for audit. """

from future import annotations import argparse import asyncio import csv import io import logging import os import random import re import smtplib import ssl import string import sys import time from concurrent.futures import ThreadPoolExecutor from contextlib import redirect_stderr, redirect_stdout from datetime import datetime from pathlib import Path from typing import Dict, List, Optional, Set, Tuple
...
EMAIL_RE = re.compile(r"[A-Za-z0-9._%+-']+@[A-Za-z0-9.-]+.[A-Za-z]{2,}", re.I) TLD_CANDIDATES = [".com", ".co.uk", ".uk", ".net", ".io", ".co", ".biz", ".org"] CONTACT_PAGES = ["/", "/contact", "/contact-us", "/about", "/about-us", "/team", "/staff", "/people", "/privacy"] ROLE_LOCALPARTS = {"admin","administrator","postmaster","hostmaster","info","contact","sales","support","help","billing","webmaster","noreply","no-reply","newsletter"} COMMON_DISPOSABLE = {"mailinator.com","tempmail.com","10minutemail.com","yopmail.com","dispostable.com"} HTML_HEURISTIC_BYTES = 512 TRANSCRIPTS_DIR = Path("transcripts") TRANSCRIPTS_DIR.mkdir(exist_ok=True)

Optional: email-validator

try: from email_validator import validate_email as ev_validate, EmailNotValidError except Exception: ev_validate = None EmailNotValidError = Exception
...
def gen_domains(company_name: str) -> List[str]: base = slug_company(company_name) out: List[str] = [] if base: for t in TLD_CANDIDATES: out.append(base + t) words = re.findall(r"[a-z0-9]+", company_name.lower()) if len(words) > 1: out.append("".join(words) + ".com") out.append(".".join(words) + ".com") seen = set() dedup: List[str] = [] for d in out: if d not in seen: dedup.append(d) seen.add(d) return dedup
...
@contextmanager def _suppress_bs4_warning(): import warnings with warnings.catch_warnings(): warnings.filterwarnings("ignore", category=MarkupResemblesLocatorWarning) yield

def is_role_local(local: str) -> bool: return local.lower() in ROLE_LOCALPARTS

def is_disposable_domain(domain: str, extras: Optional[Set[str]] = None) -> bool: s = set(COMMON_DISPOSABLE) if extras: s |= extras return domain.lower() in s

def random_localpart() -> str: return "probe" + "".join(random.choices(string.ascii_lowercase + string.digits, k=8))

----------------- MX cache (sync resolver in threadpool) -----------------

class MXCache: def init(self, mx_timeout: float = 6.0, threads: int = 8): self.mx_timeout = float(mx_timeout) self._cache: Dict[str, List[Tuple[int, str]]] = {} self._executor = ThreadPoolExecutor(max_workers=threads)
...
----------------- SMTP probe (blocking via smtplib in threadpool) -----------------

class SMTPProbeResult: def init(self, code: Optional[int], message: Optional[str], transcript: str, port: int, used_tls: bool): self.code = code self.message = message self.transcript = transcript self.port = port self.used_tls = used_tls

class SMTPProber: def init(self, smtp_timeout: float = 8.0, threads: int = 20): self.smtp_timeout = float(smtp_timeout) self._executor = ThreadPoolExecutor(max_workers=threads)
...
def _probe_sync(self, host: str, port: int, mail_from: str, rcpt_to: str, try_starttls: bool, implicit_tls: bool, timeout: float) -> SMTPProbeResult:
    # run blocking smtplib operations and capture debug output
    buf = io.StringIO()
    used_tls = False
    code = None
    message = ""
    context = ssl.create_default_context()
    # choose SMTP or SMTP_SSL
    try:
        if implicit_tls:
        ...
        # capture stdout/stderr from smtplib debug prints
        with redirect_stdout(buf), redirect_stderr(buf):
            try:
                smtp.ehlo()
            except Exception:
                pass
            # attempt STARTTLS if allowed and requested
            try:
                if try_starttls and (not implicit_tls):
                    if smtp.has_extn('starttls'):
                        try:
                            ...
                            # starttls failed; continue without TLS
                            pass
            except Exception:
                pass
            # MAIL FROM
            try:
                smtp.docmd(f"MAIL FROM", f"<%s>" % mail_from)
            except Exception:
                pass
            # RCPT TO
            try:
                rcpt_code, rcpt_msg = smtp.docmd("RCPT TO", f"<%s>" % rcpt_to)
...
class Verifier: def init(self, args): self.args = args self.mx_cache = MXCache(mx_timeout=args.mx_timeout, threads=8) self.prober = SMTPProber(smtp_timeout=args.smtp_timeout, threads=32) # rate limiting self.global_semaphore = asyncio.Semaphore(args.global_limit) self.per_host_semaphores: Dict[str, asyncio.Semaphore] = {} self.per_host_min_interval: Dict[str, float] = {} self.last_probe_at: Dict[str, float] = {} self.catch_all_cache: Dict[str, bool] = {} # mail-from rotation self.mail_froms = self._load_mail_froms(args.mail_from_list) self.mail_idx = 0 # disposable set self.disposable_set: Set[str] = set() if args.disposable_file: try: with open(args.disposable_file, "r", encoding="utf-8") as f: for ln in f: self.disposable_set.add(ln.strip().lower()) except Exception: LOG.warning("Could not load disposable list; proceeding with built-in list")
...
async def _probe_mx_host(self, host: str, ports: List[int], mail_from: str, email: str) -> Optional[SMTPProbeResult]:
    ...
    async with self.global_semaphore:
        async with sem:
            ...
            for port in ports:
                # decide whether implicit TLS (port 465) or starttls attempt
                ...
                # if connect failed, try next port
                if res.code is None and "connect-error" in (res.message or ""):
                    continue
                # return the first meaningful result (code present or transcript)
                return res
    return None

async def verify_address(self, email: str, mx_pairs: List[Tuple[int, str]]) -> Dict:
    """Returns enriched dict with scoring, transcript path, status"""
    if not mx_pairs:
    ...
    # attempt verifying against MX hosts
...
async def scrape_for_emails(session: aiohttp.ClientSession, domain: str, http_timeout: float) -> Set[str]: found: Set[str] = set() schemes = ["https://", "http://"] for scheme in schemes: base = scheme + domain.rstrip("/") for p in ["/", "/contact", "/contact-us", "/about", "/team"]: url = base + p try: async with session.get(url, timeout=aiohttp.ClientTimeout(total=http_timeout)) as resp: if resp.status != 200: continue txt = await resp.text(errors="replace") if not _looks_like_html(txt): continue with _suppress_bs4_warning(): soup = BeautifulSoup(txt, "html.parser") for a in soup.select("a[href^=mailto]"): href = a.get("href", "") m = re.search(r"mailto:([^?]+)", href, re.I) if m: found.add(m.group(1).strip().lower()) for e in EMAIL_RE.findall(soup.get_text(" ", strip=True)): found.add(e.lower()) if found: return found except asyncio.CancelledError: raise except Exception: continue return found

----------------- Syntax check -----------------

def validate_syntax(email: str) -> Tuple[bool, Optional[str]]: if not email or "@" not in email: return False, None try: if ev_validate: info = ev_validate(email, check_deliverability=False) return True, info["email"] m = EMAIL_RE.fullmatch(email.strip()) return (m is not None, email.strip() if m else None) except Exception: return False, None

----------------- Enricher pipeline (queue + retry scheduler) -----------------

class Enricher: def init(self, args): self.args = args self.verifier = Verifier(args) self.http_timeout = args.http_timeout self.retries_queue: List[Tuple[float, Dict]] = []  # (run_at, payload) self.retry_lock = asyncio.Lock()

async def _retry_scheduler(self, main_queue: asyncio.Queue, stop_event: asyncio.Event):
    # periodically move due retry items back to main_queue
...
async def _process_row(self, row: Dict[str,str], session: aiohttp.ClientSession) -> Optional[Dict[str,str]]:
    ...
    for cand in candidates:
        ...
        # perform SMTP verify (with retry handling for temporary 4xx)
        ...
        if status == "temporary":
            # schedule retry up to N times
            retries = row.get("_retries", 0)
            if retries < self.args.temporary_retries:
                # set retry-at with exponential backoff
                ...
                # mark as unknown after retries exhausted
                enriched = dict(row)
...
def parse_args(): p = argparse.ArgumentParser(description="Enricher — MX + SMTP + STARTTLS + retries + scoring") p.add_argument("--input", required=True) p.add_argument("--output", required=True) p.add_argument("--concurrency", type=int, default=30) p.add_argument("--smtp-timeout", type=float, default=8.0) p.add_argument("--mx-timeout", type=float, default=6.0) p.add_argument("--http-timeout", type=float, default=8.0) p.add_argument("--mail-from", default="verifier@example.com") p.add_argument("--mail-from-list", default=None, help="Path to file with one MAIL FROM per line (rotated)") p.add_argument("--user-agent", default="Mozilla/5.0 (compatible; Enricher/1.0)") p.add_argument("--disposable-file", default=None) p.add_argument("--skip-smtp", action="store_true", help="Skip SMTP probes (useful if port 25 blocked)") p.add_argument("--ports", default="25", help="Comma-separated ports to try (e.g. 25,587,465)") p.add_argument("--per-host-limit", type=int, default=2) p.add_argument("--per-host-interval", type=float, default=1.0, help="Minimum seconds between probes to same host") p.add_argument("--global-limit", type=int, default=50) p.add_argument("--temporary-retries", type=int, default=2, help="Retries for 4xx temporary codes") p.add_argument("--temporary-backoff", type=float, default=30.0, help="Base backoff seconds for temporary retries; exponential") return p.parse_args()

def main(): args = parse_args() # normalize ports args.ports = [int(x.strip()) for x in args.ports.split(",") if x.strip().isdigit()] args.per_host_limit = getattr(args, "per_host_limit", 2) args.per_host_interval = getattr(args, "per_host_interval", 1.0) args.global_limit = getattr(args, "global_limit", 50) args.temporary_retries = getattr(args, "temporary_retries", 2) args.temporary_backoff = getattr(args, "temporary_backoff", 30.0)
